...
class Trie:
    ...
    def _dfs(self, node: Node, currentWord: str, wordSearched: str):
        if currentWord == wordSearched:
            return True
        nextLetter = wordSearched[len(currentWord)]
        rightEdges = node.getRightEdges()
        rightEdgesAvailable = list(filter(la))
