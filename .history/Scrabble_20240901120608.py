...
def drawBoard(board: Board) -> None:
        ...
        for row in range(15):
                for col in range(15):
                        ...
                        tileFilename = letterToTileFilename(letter)
...
def letterToTileFilename(letter: str, isBoardSize: bool) -> str:
        LETTERS_TO_TILE_FILENAMES: dict = {"A": [], "B": [], "C": [], "D": [], "E": [], "F": [], "G": [], "H": [], "I": [], "J": [], "K": [], "L": [], "M": [], "N": [],  "O": [], "P": 3,"Q": 10, "R": 1, "S": 1, "T": 1, "U": 1, "V": 4, "W": 4, "X": 8, "Y": 4, "Z": 10, "BLANK": 0}
        folderPath: str
        if isBoardSize:
        ...
        for (dir_path, dir_names, file_names) in os.walk(folderPath):
                print(file_names)
                fileNamesSorted = list(filter(lambda x: x[0] == letter, file_names))
                tileFilename: str = choice(fileNamesSorted)
                return tileFilename
