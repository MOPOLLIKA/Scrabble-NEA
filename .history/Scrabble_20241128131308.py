...
class Player:
    ...
    def __init__(self) -> None:
        ...
        self._score: int = 0
        self.__temporaryScore: int = 0
        self._id: int = Player.ID
        self.__name: str = f"Player {self._id}" # implement name selection later in the settings TODO
        self.__active: bool = False
        self.__placingLetter: bool = False
        self._bot: bool = False
        Player.ID += 1

    def __repr__(self) -> str:
        return f"Player {self._id}, Name: {self.name}, Score: {self.score}, Letters: {self.letters}, isActive: {self.active}\n"
    ...
    def takeLetters(self, letterBag: LetterBag) -> None | bool:
        for _ in range(7 - len(self.__letters)):
    ...
    def setTemporaryScore(self, score: int) -> None:
        self.__temporaryScore = score

    def switchActive(self) -> None:
        self.__active = not self.__active

    def switchLetterPlacement(self) -> None:
        self.__placingLetter = not self.__placingLetter
    ...
    def isPlacingLetter(self) -> bool:
        return self.__placingLetter
    
    def getTemporaryScore(self) -> int:
        return self.__temporaryScore
...
class Bot(Player):
    ...
    def makeTurn(self, board: Board) -> bool:
        ...
        optimalWords = trie.generateOptimalWord(letters=self.getLetters(), difficulty=self.__difficulty)
        t = time.time()
