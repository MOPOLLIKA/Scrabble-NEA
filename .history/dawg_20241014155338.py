...
class Trie:
    ...
    def findNextOptimalWord(self, letters: str, possibleWords: list = [], difficulty: int = 6, prohibited: list = []) -> list[str]:
        ...
        for length in range(2, len(letters) + 1):
            ...
            for combination in combinations(letters, length):
                ...
                maxScore, maxScoreIndex = searchMaxScore(possibleWords)
                optimalWord = possibleWords[maxScoreIndex]
                if maxScore >= difficulty and optimalWord not in prohibited:
                    yield possibleWords[maxScoreIndex]
                    prohibited.append(optimalWord)
...
if __name__ == "__main__":
    ...
    trie.assemble(words)
    t1 = time.time()
    print(trie.findOptimalWord(["A", "C", "T", "D", "M", "E", "W"]))
    print(time.time() - t1)
