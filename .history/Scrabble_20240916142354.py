...
class Player:
        ...
        def __repr__(self) -> str:
                return f"Player {self.id}, Name: {self.name}, Score: {self._score}, Letters: {self.letters}, isActive: {self.active}\n"
        ...
        def setTemporaryScore(self, _score: int) -> None:
                self._temporaryScore = _score
...
class Turn:
        ...
        def calculateScore(self, board: Board) -> int:
                """
                To calculate _score, every move(tile placed) is inspected to find if there are any connected words to it horizontally
                ...
                work properly - multiplicators don't work twice) and what _score does this letter represent. Then the _score of each 
...
def drawRacks(playerQueue: PlayerQueue) -> None:
        ...
        for j in range(2):
                for i in range(2):
                        ...
                        drawRack(xPos, yPos, player, player._temporaryScore)
...
def drawRack(xPos: float, yPos: float, player: Player, _temporaryScore: int) -> None:
        ...
        text: str
        if _temporaryScore == 0:
                text = f"Score: {player.getScore()}"
        else:
                text = f"Score: {player.getScore()} + {_temporaryScore}"
...
def launchGame() -> None:
        ...
        while running:
                # variable to capture how much _score is currently being gained
                _temporaryScore: int = 0
                        ...
                        # Managing pressed keys
                                ...
                                if isPointingAtBoard(x, y) and playerCurrent.isPlacingLetter():
                                        ...
                                        if board.isValid() and turn.isValid():
                                                _temporaryScore = turn.calculateScore(board)
                                                playerCurrent.setTemporaryScore(_temporaryScore)
