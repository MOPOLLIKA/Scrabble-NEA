...
def drawRack(xPos: float, yPos: float, name: str, numberOfTiles: int) -> None:
        ...
        nameText = mediumText.render(name, True, WHITE)
        nameTextRect = nameText.get_rect()
        nameTextRect.center = (xPos + rackLength / 2, yPos - 30)
        screen.blit(nameText, nameTextRect)

        scoreText = mediumText.render(f"Score: {0}", True, WHITE)
        scoreTextRect = scoreText.get_rect()
        scoreTextRect.center = (xPos + rackLength / 2, yPos + rackHeight + 30)
        screen.blit(scoreText, scoreTextRect)

        tileSide = tileSide - (24 / 7)
        blankTileFilename = "TileImages/A1.png"
        tileImage = pygame.image.load(blankTileFilename)
        if tileImage.get_width() != tileImage.get_height() or tileImage.get_width() != tileSide:
                """
                image = Image.open(blankTileFilename)
                resized = image.resize((tileSide, tileSide))
                filenameResized = filenameResized(blankTileFilename)
                with open(filenameResized, "w") as f:
                """
                tileImage: pygame.Surface = pygame.transform.scale(tileImage, (tileSide, tileSide))
        tileImageRect: pygame.Rect = tileImage.get_rect()
        tileImageRect.center = (0, 0)
