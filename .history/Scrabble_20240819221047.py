...
class Board:
        ...
        def searchForWords(self) -> list:
                ...
                wordCurrent: str = ""
                for rowElements in self.board:
                        wordsFound += findWordsInRow(rowElements)

def findWordsInRow(rowElements) -> tuple[list, bool]:
        row: str = "".join(rowElements)
        words: list = [element for element in row.split(" ") if element != "" and isWord(element)[0]]
        isCorrect: bool = True if False not in map(lambda x: isWord(x)[0], words) else False
