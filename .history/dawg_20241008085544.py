...
class Trie:
    ...
    def _searchRight(self, node: Node, word: str, depth: int) -> bool:
        ...
        letter = word[depth]
        rightEdges = list(filter(lambda x: x == letter and not x.getRightNode().getIsSearched(), rightEdges))
        # Stopping criterion
        ...
        if len(rightEdges) == 0:
            ...
            return self._searchRight(previousNode, word, depth - 1)
        if choice == -1:
            return False

        edge = rightEdges[choice]
        nextNode = edge.getRightNode()
        print(nextNode)
        return self._searchRight(nextNode, node, word, depth + 1)
