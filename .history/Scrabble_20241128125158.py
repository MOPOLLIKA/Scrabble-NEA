...
class Player:
    ...
    def __init__(self) -> None:
        ...
        self.__id: int = Player.ID
        self.__name: str = f"Player {self.id}" # implement name selection later in the settings TODO
        self.__active: bool = False
        self.__placingLetter: bool = False
        self.__bot: bool = False
        Player.ID += 1
    ...
    def applyMoves(self, board: Board, moves: list[tuple]) -> None:
        for col, row, letter in moves:
            self.__placeLetter(letter, col, row, board)
