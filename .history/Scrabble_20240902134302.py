...
class PlayerQueue:
        def __init__(self, players: Iterable[Player]):
                ...
                self.originalQueue: list[Player] = self.queue.copy()
                self.length: int = len(self.queue)
        
        def nextTurn(self):
                playerNext: Player = self.queue.pop(0)
                self.queue.insert(self., playerNext)
                return playerNext
        
        def getOriginalQueue(self):
...
def drawRacks(names: dict[str: int], playerQueue: list[Player]) -> None:
        """Variable `names` is in the following format: {'name1': numberOfTiles1, 'name2': numberOfTiles2, ...}"""
        count: int = len(playerQueue)
        for i in range(2):
                for j in range(2):
                        if count == 0:
                                break
