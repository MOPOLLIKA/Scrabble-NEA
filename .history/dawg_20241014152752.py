...
def findCombinationsOfLetters(elements: list[str], number: int) -> list[str]:
    for separation in range(1, len(elements)):
...
class Trie:
    ...
    def findPossibleWords(self, letters: str, possibleWords: list = [], difficulty: int = 4) -> list[str]:
        ...
        for length in range(2, len(letters) + 1):
            combinations = findCombinationsOfElements(letters, length)
    
    def searchConstantLengthWords(self, letters: str) -> list[str]:
