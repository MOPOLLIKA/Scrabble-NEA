...
class Trie:
    ...
    def _searchRight(self, node: Node, word: str, depth: int, choice: int = 0, previousNode: Node = None) -> bool:
        rightEdges = node.getRightEdges()
        letter = word[depth]
        rightEdges = filter(lambda x: x == letter,)
        # Stopping criterion
        if depth == len(word) - 1 and node.getIsTerminal():
            return True
        # False if no next letter is found
        if letter not in rightEdges:
            return self._searchRight(previousNode)
