...
def isPointingAtBoard(x: int, y: int) -> bool:
        leftBoardBorderX: float = 0.5 * WIDTH - 0.35 * HEIGHT
        topBoardBorderY: float = 0.15 * HEIGHT

def tilePointing(x: int, y: int) -> tuple[int, int]:
        """Returns tuple (col, row) of the board tile the player is pointing at."""
        leftBoardBorderX: float = 1
...
def launchGame() -> None:
        ...
        while running:
                ...
                info = bigText.render(f"{pygame.mouse.get_pos()}", True, WHITE)
