...
def rackPosition(i: int, j: int) -> tuple[int, int]:
        length: float = 0.2 * WIDTH
        rackheight: float = length / 7 + 10
        if i == 0:
                yPos = 0.2 * HEIGHT
        else:
                yPos = 0.8 * HEIGHT - height

        if j == 0:
                xPos = (0.5 * WIDTH - 0.35 * HEIGHT) / 2 - 0.1 * WIDTH
        else:
                xPos = (0.5 * WIDTH + 0.35 * HEIGHT) + (0.5 * WIDTH - 0.35 * HEIGHT) / 2 - 0.1 * WIDTH
        return xPos, yPos
        
def drawRack(xPos: float, yPos: float, index: int, numberOfTiles: int) -> None:
        length: float = 0.2 * WIDTH
        height: float = length / 7 + 10
...
def launchGame() -> None:
        ...
        title = bigText.render("Scrabble Ultra", True, (255, 255, 255))
